...
flow.SetAlias("(.*)_p4","{TLorentzVector ret; ret.SetPtEtaPhiM(\\1_pt,\\1_eta,\\1_phi,\\1_mass); return ret;}",defaultPersistency=False)
...
flow.DefineCollAttr("Muon_id","Muon_tightId*3+Muon_looseId") #Maps to plain RDF VecOps
flow.SubCollection("SelectedMuon","Muon",sel="Muon_iso < muIsoCut && Muon_id > muIdCut && Muon_pt > muPtCut") #this should generate some kind of wrapper/ref that can be used as the parent collection

flow.Filter("twoOppositeSignMuons","nSelectedMuon==2 && SelectedMuon_charge[0]*SelectedMuon_charge[1] < 0")
flow.Define("Higgs","p4at(SelectedMuon,0)+p4at(SelectedMuon,1)",requires=["twoOppositeSignMuons"]) # p4 should be handled somehow ... any syntax is ok such as p4(SelectedMuon[0]) or _p4 or .p4 etc..
