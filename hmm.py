...
def buildFlow(flow):
    ...
    flow.TakePair("Mu", "SelectedMuon", "MuMu",
                  "At(OppositeSignMuMu,0,-200)", requires=["twoOppositeSignMuons"])
    ...
    flow.ObjectAt("QJet0", "SelectedJet",
                  'At(SortedSelectedJetIndices,0)', requires=["twoJets"])
    flow.ObjectAt("QJet1", "SelectedJet",
                  'At(SortedSelectedJetIndices,1)', requires=["twoJets"])
    ...
    flow.Define(
        "qqDeltaPhi", "abs(ROOT::Math::VectorUtil::DeltaPhi(QJet0_p4,QJet1_p4))")

    # QQ vs ll kinematic
    flow.Define(
        "ll_ystar", "Higgs.Rapidity() - (QJet0_p4.Rapidity() + QJet1_p4.Rapidity())/2.f")
    flow.Define(
        "ll_zstar", " abs( ll_ystar/ (QJet0_p4.Rapidity()-QJet1_p4.Rapidity() )) ")
    ...
    flow.Define(
        "PhiHQ1", "abs(ROOT::Math::VectorUtil::DeltaPhi(Higgs,QJet0_p4))")
    flow.Define(
        "PhiHQ2", "abs(ROOT::Math::VectorUtil::DeltaPhi(Higgs,QJet1_p4))")
    flow.Define("EtaHQ1", "abs(Higgs.Eta() - QJet0_eta)")
    flow.Define("EtaHQ2", "abs(Higgs.Eta() - QJet1_eta)")
    flow.Define(
        "DeltaRelQQ", "(QJet0_p4+QJet1_p4).Pt()/( QJet0_p4.Pt()+QJet1_p4.Pt())")
    flow.Define(
        "Rpt", "(QJet0_p4+QJet1_p4+ Higgs).Pt()/( QJet0_p4.Pt()+QJet1_p4.Pt() + Higgs.Pt())")
    flow.Define("mmjj", "Higgs+qq")
    flow.Define(
        "theta2", "Higgs.Vect().Dot(QJet1_p4.Vect())/QJet1_p4.Vect().R()/Higgs.Vect().R()")
    flow.Define("SortedSelectedMuonIndices", "Argsort(-SelectedMuon_pt)")
    flow.ObjectAt("LeadMuon", "SelectedMuon",
                  "SortedSelectedMuonIndices[0]", requires=["twoMuons"])
    flow.ObjectAt("SubMuon", "SelectedMuon",
                  "SortedSelectedMuonIndices[1]", requires=["twoMuons"])
    ...
    flow.Define("MaxJetAbsEta",
                "std::max(std::abs(QJet0_eta), std::abs(QJet1_eta))")
    flow.Define("minEtaHQ", "std::min(abs(EtaHQ1),(EtaHQ2))")
    flow.Define("minPhiHQ", "std::min(abs(PhiHQ1),abs(PhiHQ2))")

    flow.DefaultConfig(higgsMassWindowWidth=10, mQQcut=400,
                       nominalHMass=125.03)  # ,btagCut=0.8)
    flow.Define("btagCut", "0.4184f")
    flow.Define("btagCutL", "0.1241f")
    # adding for sync
    flow.Define(
        "nbtagged", "int(Nonzero(SelectedJet_btagDeepB > btagCut && abs(SelectedJet_eta)< 2.5).size())")
    flow.Define(
        "nbtaggedL", "int(Nonzero(SelectedJet_btagDeepB > btagCutL && abs(SelectedJet_eta)< 2.5).size())")
...
#    flow.Define("nelectrons","int(Nonzero(Electron_pt > 20 && abs(Electron_eta) < 2.5 && Electron_mvaFall17V2Iso_WP90 ).size())")
    ...
    flow.Selection(
        "MassWindow", "abs(Higgs.M()-nominalHMass)<higgsMassWindowWidth")
    flow.Selection("MassWindowZ", "abs(Higgs.M()-91)<15")
    flow.Selection(
        "VBFRegion", "Mqq > mQQcut && QJet0_pt> 35 && QJet1_pt > 25")
    flow.Selection("PreSel", "nelectrons==0 && nbtaggedL < 2 && VBFRegion && twoOppositeSignMuons && nbtagged < 1 && LeadMuon_pt > 26 && SubMuon_pt > 20  && abs(SubMuon_eta) <2.4 && abs(LeadMuon_eta) < 2.4", requires=[
                   "VBFRegion", "twoOppositeSignMuons"])
    flow.Selection("PreSelSimple", "VBFRegion && twoOppositeSignMuons && LeadMuon_pt > 26 && SubMuon_pt > 20  && abs(SubMuon_eta) <2.4 && abs(LeadMuon_eta) < 2.4", requires=[
                   "VBFRegion", "twoOppositeSignMuons"])
    flow.Selection("SideBand", "Higgs_m < 150 && Higgs_m > 110 && ! MassWindow && VBFRegion &&  qqDeltaEta > 2.5", requires=[
                   "VBFRegion", "PreSel"])
    flow.Selection("SignalRegion", "VBFRegion && MassWindow &&  qqDeltaEta > 2.5", requires=[
                   "VBFRegion", "MassWindow", "PreSel"])
    flow.Selection("ZRegion", "VBFRegion && MassWindowZ  && qqDeltaEta > 2.5", requires=[
                   "VBFRegion", "MassWindowZ", "PreSel"])
    return flow


flow = SampleProcessing(
    "Simple Test", "/gpfs/ddn/users/fvaselli/muonData/synth_nanoaod.root")
flow = buildFlow(flow)

flow1 = SampleProcessing(
...
targets = ["SelectedMuon", "Mqq_log", "Rpt", "qqDeltaEta", "ll_zstar_log", "minEtaHQ", "Higgs_pt", "Higgs_eta", "Mqq",
           "QJet0_pt", "QJet1_pt", "QJet0_eta", "QJet1_eta", "QJet0_phi", "QJet1_phi", "QJet0_qgl", "QJet1_qgl", "Higgs_m"]
...
processor1 = flow1.CreateProcessor(
    "eventProcessor1", targets, histos, [], "", nthreads)
...
for sam in ["DY", "HMM", "EWK"]:
    rdf = ROOT.RDataFrame(
        "Events", "/gpfs/ddn/users/fvaselli/muonData/synth_%s_nanoaod.root" % sam)
    rdf1 = ROOT.RDataFrame(
        "Events", "/gpfs/ddn/users/fvaselli/nanoaods/%s1.root" % sam)
    ...
    for (o, s) in zip(result1.histos, result.histos):
        ...
        s.SetLineColor(2)
        s.Draw("sames")
        cc[-1].SaveAs("/afs/pi.infn.it/user/arizzi/public_html/flashsim/%s--%s.png" %
                      (sam, o.GetName()))
