...
flow.Selection("twoMuons","nSelectedMuon>=2",requires=["oneMuon"]) #here "requires" actually means "implies"... to be adjusted 
...
flow.SubCollection("SelectedJet","Jet","Jet_pt > jetPtCut && (Jet_LeptonIdx==-1 || Jet_LeptonDr > 0.3)") #pt cut plus DeltaR cleaning
flow.MatchDeltaR("GenJet","SelectedJet")
flow.Define("GenJet_p4","@p4v(GenJet)")

flow.Define("JetSum_p4","std::accumulate(SelectedJet_p4.begin(), SelectedJet_p4.end(), ROOT::Math::PtEtaPhiMVector())") #Feature request=> Sum with non T(0) start value
...
flow.VariationWeightArray("LHEScaleWeight",8,filt=lambda selname,hname,wname : "__syst__" not in hname and  "__syst__" not in selname ) 
...
snap=genericHistos+signalRegionHistos+["nMuon"]+[x for x in flow.validCols if x[:5]=="Muon_"]
print "columns selected for snapshot ntuple", snap
flow.printRDFCpp(snap,debug=False,outname=sys.argv[1],selections=histosWithSystematics,snap=snap,snapsel="twoOppositeSignMuons")
