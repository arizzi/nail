...
flow = SampleProcessing("VBF Hmumu Analysis",
                        "00346AC4-1BF1-BC41-AD23-D3D65D027293.root")
...
flow.Define("GenHiggs_idx", "Nonzero(GenPart_pdgId == 25)",
            requires=["hasHiggs"])
flow.SubCollection("QParton", "GenPart",
                   sel="GenPart_genPartIdxMother==At(Take(GenPart_genPartIdxMother,GenHiggs_idx),0,-100) && GenPart_pdgId!= 25")
...
flow.TakePair("QJet", "SelectedJet", "JetPair",
              "Argmax(MemberMap((JetPair0_p4+JetPair1_p4),M() ))", requires=["twoJets"])
...
# QQ vs ll kinematic
flow.Define(
    "ll_ystar", "Higgs.Rapidity() - (QJet0_p4.Rapidity() + QJet1_p4.Rapidity())")
flow.Define(
...
flow.Selection("SignalRegion", "VBFRegion && MassWindow",
               requires=["VBFRegion", "MassWindow"])
flow.Selection("TwoJetsTwoMu", "twoJets && twoOppositeSignMuons",
               requires=["twoJets", "twoOppositeSignMuons"])
...
flow.Define("SBClassifier", "Higgs_pt+Higgs_m+Mqq+Rpt+DeltaRelQQ+NSoft2")


# define some event weights
flow.Define("SelectedJet_btagWeight",
            "vector_map(btagWeight,SelectedJet_btagCSVV2,SelectedJet_pt,SelectedJet_eta)")
flow.Define("btagEventWeight",
            "std::accumulate(SelectedJet_btagWeight.begin(),SelectedJet_btagWeight.end(),1, std::multiplies<double>())")
...
flow.Define("muEffWeight",
            "effMu2016(LeadMuon_pt,LeadMuon_eta)*effMu2016(SubMuon_pt,SubMuon_eta)")
flow.CentralWeight("muEffWeight", ["twoMuons"])


# Systematic weights
# systematic variations are 1D, let's avoid systematics of systematic
flow.VariationWeightArray("LHEScaleWeight", 8, filt=lambda sname,
                          hname, wname: "__syst__" not in hname and "__syst__" not in sname)
...
flow.Define("SelectedJet_btagWeight_up",
            "vector_map(btagWeightUp,SelectedJet_btagCSVV2,SelectedJet_pt,SelectedJet_eta)")
...
flow.Systematic("BTagUp", "SelectedJet_btagWeight",
                "SelectedJet_btagWeight_up")
...
# this should be protected against systematic variations
...
for i in range(5):
    ...
    flow.Systematic("JEC%s" % i, "Jet_pt", "Jet_pt_JEC%s" %
                    i)  # name, target, replacement


# define hist mapping
genericHistos = ["LeadMuon_pt", "LeadMuon_eta", "SubMuon_pt",
                 "SubMuon_eta", "QJet0_pt", "QJet1_pt", "QJet0_eta", "QJet1_eta"]
...
snap = ["SideBand", "nSoftActivityJet", "SoftActivityJet_pt", "SoftActivityJet_eta", "SoftActivityJet_phi", "SoftActivityJet_SelectedJetDr", "SoftActivityJet_SelectedJetIdx", "SoftActivityJet_SelectedMuonDr",
        "SoftActivityJet_SelectedMuonIdx", "VBFRegion", "QJet0_pt", "QJet0_eta", "QJet0_btagCSVV2", "QJet1_pt", "QJet1_eta", "QJet1_btagCSVV2", "Mu0_pt", "Mu0_eta", "Mu1_pt", "Mu1_eta", "QJet0", "QJet1", "qqDeltaEta", "MqqGenJet"]
flow.printRDFCpp(snap, debug=False,
                 outname=sys.argv[1], selections=histosWithSystematics, snap=snap, snapsel="TwoJetsTwoMu")
