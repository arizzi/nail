...
flow.SubCollection("LooseEle", "Electron",
                   sel="Electron_iso < eIsoCut && Electron_id > eIdCut && Electron_pt > ePtCut")
...
# this allows to later just use "btag" and to study changes of btag algorithm
flow.Define("CleanJet_btag", "CleanJet_btagCSVV2")
flow.Define("CleanJet_p4", "@p4v(CleanJet)")


# * Compute event variables using selected cleaned jets
# Feature request=> Sum with stardvalue
flow.Define("JetSum_p4",
            "std::accumulate(CleanJet_p4.begin(), CleanJet_p4.end(), ROOT::Math::PtEtaPhiMVector())")
# Feature request=> Sum with stardvalue
flow.Define("LeptonSum_p4",
            "std::accumulate(Lepton_p4.begin(), Lepton_p4.end(), ROOT::Math::PtEtaPhiMVector())")
...
#    * Closest to Z
flow.TakePair("ZLep", "Lepton", "LPair",
              "Argmax(-abs(MemberMap((LPair0_p4+LPair1_p4),M() )-91.2)*isOSSF)", requires=["hasOSSF"])
flow.Define("Z", "ZLep0_p4+ZLep1_p4")
flow.Define("Z_mass", "Z.M()")
#    * Highest pt pair
flow.TakePair("LepHPt", "Lepton", "LPair",
              "Argmax((MemberMap((LPair0_p4+LPair1_p4),pt() ))*isOSSF)", requires=["hasOSSF"])
flow.Define("OSSFHPt", "LepHpt0_p4+LepHPt1_p4")
flow.Define("OSSFHPt_mass", "OSSFHPt.M()")

#    * Highest mass pair
flow.TakePair("LepHM", "Lepton", "LPair",
              "Argmax(MemberMap((LPair0_p4+LPair1_p4),M() )*isOSSF)", requires=["hasOSSF"])
flow.Define("OSSFHM", "LepHM0_p4+LepHM1_p4")
flow.Define("OSSFHM_mass", "OSSFHM.M()")

# * Find opposite sign, opposite flavour pairs
flow.Define("isOSOF", "LPair0_charge != LPair1_charge && LPair0_pid != LPair1_pid",
            requires=["twoLeptons"])
flow.Selection("hasOSOF", "Sum(isOSOF)")
#    * Highest pt pair
flow.TakePair("LepOFHPt", "Lepton", "LPair",
              "Argmax((MemberMap((LPair0_p4+LPair1_p4),pt() ))*isOSOF)", requires=["hasOSOF"])
flow.Define("OSOFHPt", "LepOFHpt0_p4+LepOFHPt1_p4")
flow.Define("OSOFHPt_mass", "OSOFHPt.M()")

#    * Highest mass pair
flow.TakePair("LepOFHM", "Lepton", "LPair",
              "Argmax(MemberMap((LPair0_p4+LPair1_p4),M() )*isOSOF)", requires=["hasOSOF"])
...
#    * Closest to W mass
flow.TakePair("JJBestWMass", "CleanJet", "JJ",
              "Argmax(-abs(MemberMap((JJ0_p4+JJ1_p4),M() )-80.4))", requires=["twoJets"])
...
flow.TakePair("JJBestLike", "CleanJet", "JJ",
              "Argmax(-(pow(MemberMap((JJ0_p4+JJ1_p4),M() )-80.4,2)/400.)-JJ0_btag-JJ1_btag)", requires=["twoJets"])
...
flow.SubCollection("TightLepton", "Lepton",
                   "Lepton_iso < tightIso && Lepton_id > tightIdCut && Lepton_pt > tightPtCut")
flow.Selection("oneTightLepton", "nTightLepton>0")
# * Trigger eff weight
#    * From leading lepton pt (no trigger obj matching)
flow.ObjectAt("LeadingLepton", "TightLepton",
              "Argsort(-TightLepton_pt)[0]", requires=["oneTightLepton"])

flow.Define("SingleLeptonEfficiency",
            "efficiency(LeadingLepton_pt,LeadingLepton_eta,LeadingLepton_pid)")
...
flow.Selection("SemileptonRegion", "nTightLepton==1 && nLepton == 1",
               requires=["oneTightLepton", "twoJets"])
...
flow.Define("CleanJet_btagWeight",
            "vector_map(btagWeight,CleanJet_btag,CleanJet_pt,CleanJet_eta)")
flow.Define("btagEventWeight",
            "std::accumulate(CleanJet_btagWeight.begin(),CleanJet_btagWeight.end(),1, std::multiplies<double>())")
...
colsToPlot = ["nJet", "nLepton", "MHT", "HT", "Z_mass",
              "OSOFHM_mass", "OSSFHM_mass", "JJBestLike_mass", "JJBestWMass_mass"]
...
flow.printRDFCpp([], debug=False, outname=sys.argv[1],
                 selections=histosWithSystematics, snap=colsToPlot, snapsel="hasOS")
